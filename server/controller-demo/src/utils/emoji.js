...
function defaultImageSrcGenerator(icon, options) {
    var base = 'https://twemoji.maxcdn.com/';
    return ''.concat(base, options.size, '/', icon, options.ext);
}
...
function parse(what, how) {
    ...
    return parseString(what, {
        ...
        ext: how.ext,
        base: how.base,
        className: how.className,
        staticHost: how.staticHost,
        size: toSizeSquaredAsset(how.size),
    });
}
 ...
 *  对外暴露的方法
 */
export default function renderEmoji(html, options) {
    return parse(html, {
        // 是否使用本地图片资源
        base: options.base || false,
        ext: options.ext || '.png',
        size: options.size || '72x72',
        className: options.className || 'ai-emoji',
        staticHost: options.staticHost || 'https://cstatic.msxf.com/anaturallr/smartxma/static',
        callback: (icon, options) => {
            let url = options.base
                ? options.staticHost + '/emoji/png'
                : 'https://twemoji.maxcdn.com/72x72';
                // return `${url}/${icon}${options.ext}`;
                return url + '/' + icon + options.ext;
                // (废弃)
                // return options.base + 'png' + '/' + icon + options.ext;
        },
    });
}